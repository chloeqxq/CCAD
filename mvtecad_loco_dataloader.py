...
class MVTecLocoDataset(Dataset):
    def __init__(self, type, root):
        self.data = []
        self.root = root
        self.label_to_idx = {'breakfast_box': '0', 'juice_bottle': '1', 'pushpins': '2', 'screw_bag': '3', 'splicing_connectors': '4'}
    ...
    def load_data(self, type, root):
        data = []
        for clsname in ['breakfast_box', 'juice_bottle', 'pushpins', 'screw_bag', 'splicing_connectors']:
            if type == 'train':
                json_filename = 'train.json'
                data.extend(self.get_json(os.path.join(root, clsname, 'train', 'good'), clsname, is_train = True))
            else:
                json_filename = 'test.json'
                test_dirs = ['good', 'logical_anomalies', 'structural_anomalies']
        ...
        self.data = data
        self.save_json(data, os.path.join(root, '..', '..', 'diad-main_copy', 'training', 'MVTec_loco', json_filename)) # 更改
        ...
        for img_name in os.listdir(dir_path):
            ...
            # testing dataset
                ...
                if test_sub_dir == 'good':
                    ...
                    mask_path = os.path.join(clsname, 'ground_truth', test_sub_dir, mask_name, '000.png')
