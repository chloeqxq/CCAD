...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # collect loss
        self.batch_wise_loss.append(loss)
    ...
    def on_validation_epoch_end(self, *args, **kwargs):
        ...
        log_message = log_metrics(ret_metrics, evl_metrics)

        # Open file and write log_metrics output
        # qixin-edit
        if log_message is None:
            log_message = "No metrics"
        with open('MVTec_DiAD_1e4_200_1000_10.txt', 'a') as file_object: # 更改文件名（与522行统一）
            file_object.write('Validation Metrics:\n')
    ...
    def on_train_epoch_end(self, *args, **kwargs):
        loss_avg = sum(self.batch_wise_loss)/ (len(self.batch_wise_loss)+0.0001)
        file_object = open('MVTec_DiAD_1e4_200_1000_10.txt', 'a') # 更改文件名（与510行统一）
        # file_object.write('Train:\tloss-{:.6f}\tlen-{}\n'.format(loss_avg,len(self.batch_wise_loss)))
        file_object.write(f'Epoch: {self.epoch_idx}, Train loss: {loss_avg:.6f}, Length: {len(self.batch_wise_loss)}\n')
        file_object.close()
        self.batch_wise_loss = []
        self.epoch_idx += 1
